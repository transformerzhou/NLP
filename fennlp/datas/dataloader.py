...
class TFWriter(object):
    def __init__(self, maxlen, vocab_files, modes, data_dir="tests/NER/NER_ZH/ner_data", task="NER", do_low_case=False,
    ...
    def label2id(self):
        ...
        for (i, label) in enumerate(self.label_list):
            label_map[label] = i
        pickle.dump(label_map, open(os.path.join(self.data_dir, "label2id.pkl"), 'wb'))
        return label_map
    ...
    def _write_examples(self, examples, task="ner"):
        output_file = os.path.join(self.data_dir, self.mode + ".tfrecords")
        ...
        writer.close()
        print("%s has been converted into %s" % (self.filename, output_file))
...
class TFLoader(object):
    def __init__(self, maxlen, batch_size, data_dir=None, task="ner", epoch=None):
        self.maxlen = maxlen
    ...
    def load_train(self):
        self.filename = os.path.join(self.data_dir, "train.tfrecords")
    ...
    def load_dev(self):
        self.filename = os.path.join(self.data_dir, "dev.tfrecords")
        raw_dataset = tf.data.TFRecordDataset(self.filename)
    ...
    def load_test(self):
        self.filename = os.path.join(self.data_dir, "test.tfrecords")
        raw_dataset = tf.data.TFRecordDataset(self.filename)
        dataset = raw_dataset.map(
            lambda record: self.decode_record(record)
        )
