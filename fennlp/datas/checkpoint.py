...
class LoadCheckpoint(object):
    ...
    def __init__(self, language='zh', model="bert",
                 parameters="base", cased=True, is_download=True, url=None, file_path=None):
        """
    ...
    def load_bert_param(self, pretraining=False):
        if self.is_download == True:
            ...
            config = os.path.join(self.file_path, "bert_config.json")
            vocab_file = os.path.join(self.file_path, "vocab.txt")
            model_path = os.path.join(self.file_path, "bert_model.ckpt")
