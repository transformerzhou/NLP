...
def convert2sentence(biofile, sentenceoutput):
    rf = codecs.open(biofile, encoding="utf-8")
    wf = codecs.open(sentenceoutput, 'w', encoding='utf-8')
    sentence = []
    label = []
    w = ''
    tag = ''
...
def conver2sentencepair(inputdir):
    file_list = glob.glob(os.path.join(inputdir, "**.txt"))
    for input_file in file_list:
        output_file = os.path.join(inputdir, input_file.split("\\")[-1].split(".")[0])
        convert2sentence(input_file, output_file)
        print("%s has been convert into %s" % (input_file, output_file))
#
